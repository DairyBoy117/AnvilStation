...
{
	echo ' <div class="updated" id="import_comp_noti" style="display:none;"><p>Success! The data has been imported successfully. USE admin/admin TO LOG IN!</p></div>';
}
function import_incompleted_notice()
{
	echo ' <div class="update-nag" id="import_incomp_noti" style="display:none;"><p>To protect your content the demo importer only works when Wordpress is fresh installed. Please re-install Wordpress with the default settings if you want to import the demo data.<a href="#" id="close_update_nag"> X</a></p></div>';
}
            ...
            $output .= '<select class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" id="' . esc_attr( $value['id'] ) . '">';
                    ...
                    if ($count > 0)
                    {
                        foreach ($terms as $term)
                        {
                             $selected = '';
                             if( $val != '' ) {
                                 if ( $val == $term->term_id) { $selected = ' selected="selected"';}
                             }
                             if($term->parent == 0 ){
                      ...
                      $output .= '<option selected value="0">' . esc_html__( 'No categories!', 'blackfyre' ) . '</option>';
...
<script type="text/javascript">
		...
		// Start jQuery goodness
			$('#itoggle input#<?php echo esc_attr($value['id']); ?>').iToggle({
				easing: 'easeOutExpo',
				onClickOn: function(){
	 ...
	 case 'impbutton':
			...
			?>
            	<script type="text/javascript">
					jQuery(document).ready(function($)
					{
						var whatever = "<?php echo esc_url($importer_url); ?>";
						var demo_stat = "<?php echo esc_attr($count); ?>";
		...
		$output .= '<div class="loading" id="target" style="display:none;"><div class="loading_text">Please wait, the content is importing....</div></div>';
