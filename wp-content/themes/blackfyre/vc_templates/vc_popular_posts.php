<?php
$el_pp_number_posts = $el_pp_categories =  $el_pp_title = $el_class = '';
$posts = array();
...
<div class="<?php echo esc_attr($css_class); if(!empty($el_class)) echo esc_attr($el_class); ?>">
    <div class="wpb_wrapper">
        <div class="title-wrapper">
            <h3 class="widget-title"><i class="fa fa-newspaper-o"></i> <?php if(!empty($el_pp_title)) echo esc_attr($el_pp_title); ?></h3>
            <div class="clear"></div>
        </div>
        <?php
                    // Categories
                    $ct = array();
                    if ( $el_pp_categories != '' ) {
                        $el_pp_categories = explode( ",", $el_pp_categories );
                        foreach ( $el_pp_categories as $category ) {
                            array_push( $ct, $category );
                        }
                    }

                    $posts = new WP_Query(array(
                        'showposts' => $el_pp_number_posts,
                        'category__in' => $ct,
                        'meta_key' => 'overall_rating' ,
                        'orderby' => array( 'comment_count' => 'DESC', 'meta_value_num' => 'DESC' )
                    ));
        ?>
