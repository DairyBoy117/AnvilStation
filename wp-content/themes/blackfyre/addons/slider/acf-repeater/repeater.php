...
class acf_field_repeater extends acf_field
{
	...
	*/
	...
	function __construct()
	{
		// vars
		$this->name = 'repeater';
		$this->label = esc_html__("Repeater",'blackfyre');
		$this->category = esc_html__("Layout",'blackfyre');
		$this->defaults = array(
			'sub_fields'	=>	array(),
			'row_min'		=>	0,
			'row_limit'		=>	0,
			'layout' 		=> 'table',
			'button_label'	=>	esc_html__("Add Row",'blackfyre'),
		);
		$this->l10n = array(
			'min'	=>	esc_html__("Minimum rows reached ( {min} rows )",'blackfyre'),
			'max'	=>	esc_html__("Maximum rows reached ( {max} rows )",'blackfyre'),
		);
