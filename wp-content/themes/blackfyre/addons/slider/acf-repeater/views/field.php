...
?>
<div class="repeater" data-min_rows="<?php echo esc_attr($field['row_min']); ?>" data-max_rows="<?php echo esc_attr($field['row_limit']); ?>">
		...
		<thead>
			<tr>
				...
				<?php foreach( $field['sub_fields'] as $sub_field ):
					...
					?>
					<th class="acf-th-<?php echo esc_attr($sub_field['name']); ?> field_key-<?php echo esc_attr($sub_field['key']); ?>" <?php echo wp_kses($attr, $blackfyre_allowed); ?>>
						<span><?php echo wp_kses($sub_field['label'] . $required_label, $blackfyre_allowed); ?></span>
						<?php if( isset($sub_field['instructions']) ): ?>
							<span class="sub-field-instructions"><?php echo wp_kses($sub_field['instructions'], $blackfyre_allowed); ?></span>
						<?php endif; ?>
					</th><?php
				endforeach; ?>
	...
	<?php if( $field['value'] ): foreach( $field['value'] as $i => $value ): ?>
		...
		if( $field['row_limit'] > 1 ): ?>
			<td class="order"><?php echo esc_attr($i+1); ?></td>
		<?php endif; ?>
		...
		foreach( $field['sub_fields'] as $sub_field ): ?>
			...
			if( $field['layout'] == 'row' ): ?>
				<tr <?php acf_join_attr( $attributes ); ?>>
					<td class="label">
						<label>
							<?php echo esc_attr($sub_field['label']); ?>
	...
	<ul class="hl clearfix repeater-footer">
		<li class="right">
			<a href="#" class="add-row-end acf-button"><?php echo esc_attr($field['button_label']); ?></a>
		</li>
	</ul>
