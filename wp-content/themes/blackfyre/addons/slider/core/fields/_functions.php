...
class acf_field_functions
{
	...
	*/
	...
	{
		$found = false;
		$cache = wp_cache_get( 'load_value/post_id=' . $post_id . '/name=' . $field['name'], 'blackfyre', false, $found );
		...
		//update cache
		wp_cache_set( 'load_value/post_id=' . $post_id . '/name=' . $field['name'], $value, 'blackfyre' );
	...
	*/
	...
	function delete_value( $post_id, $key )
	{
		...
		wp_cache_delete( 'load_value/post_id=' . $post_id . '/name=' . $key, 'blackfyre' );
	}
	...
	*/
	...
	function load_field( $field, $field_key, $post_id = false )
	{
		...
		if( !$field )
		{
			$field = wp_cache_get( 'load_field/key=' . $field_key, 'blackfyre' );
		}
		...
		if( !$field )
		{
			...
			if( !empty($rows) )
			{
				...
				*/
				...
				{
					$wpml_post_id = icl_object_id($row['post_id'], 'blackfyre', true, ICL_LANGUAGE_CODE);
		...
		// set cache
		wp_cache_set( 'load_field/key=' . $field_key, $field, 'blackfyre' );
	...
	*/
	...
	function update_field( $field, $post_id )
	{
		...
		// clear cache
		wp_cache_delete( 'load_field/key=' . $field['key'], 'blackfyre' );
