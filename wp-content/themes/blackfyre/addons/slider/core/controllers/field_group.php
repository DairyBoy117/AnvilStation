...
class acf_field_group
{
	...
	*/
	...
	function get_field_groups( $array )
	{
		// cache
		$found = false;
		$cache = wp_cache_get( 'field_groups', 'blackfyre', false, $found );
		...
		// set cache
		wp_cache_set( 'field_groups', $array, 'blackfyre' );
	...
	*/
	...
	function admin_head()
	{
		...
		// l10n
		$l10n = array(
			'move_to_trash'		=>	esc_html__("Move to trash. Are you sure?",'blackfyre'),
			'checked'			=>	esc_html__("checked",'blackfyre'),
			'no_fields'			=>	esc_html__("No toggle fields available",'blackfyre'),
			'title'				=>	esc_html__("Field group title is required",'blackfyre'),
			'copy'				=>	esc_html__("copy",'blackfyre'),
			'or'				=>	esc_html__("or",'blackfyre'),
			'fields'			=>	esc_html__("Fields",'blackfyre'),
			'parent_fields'		=>	esc_html__("Parent fields",'blackfyre'),
			'sibling_fields'	=>	esc_html__("Sibling fields",'blackfyre'),
			'hide_show_all'		=>	esc_html__("Hide / Show All",'blackfyre')
		);
...
(function($) {
	...
	// vars
	acf.post_id = <?php echo esc_attr($post->ID); ?>;
	...
	acf.l10n = <?php echo json_encode( $l10n ); ?>;
		...
		// add metaboxes
		add_meta_box('acf_fields', esc_html__("Fields",'blackfyre'), array($this, 'html_fields'), 'blackfyre', 'normal', 'high');
		add_meta_box('acf_location', esc_html__("Location",'blackfyre'), array($this, 'html_location'), 'blackfyre', 'normal', 'high');
		add_meta_box('acf_options', esc_html__("Options",'blackfyre'), array($this, 'html_options'), 'blackfyre', 'normal', 'high');
	...
	*/
	...
	{
	    $current .= '<h5>' . esc_html__("Fields",'blackfyre') . '</h5>';

	    $current .= '<div class="show-field_key">' . esc_html__("Show Field Key:",'blackfyre');
			$current .= '<label class="show-field_key-no"><input checked="checked" type="radio" value="0" name="show-field_key" />' . esc_html__("No",'blackfyre') . '</label>';
			$current .= '<label class="show-field_key-yes"><input type="radio" value="1" name="show-field_key" />' . esc_html__("Yes",'blackfyre') . '</label>';
		$current .= '</div>';
	...
	function ajax_render_location( $options = array() )
	{
		...
		switch($options['param'])
		{
			...
			case "page_type" :
				...
				$choices = array(
					'front_page'	=>	esc_html__("Front Page",'blackfyre'),
					'posts_page'	=>	esc_html__("Posts Page",'blackfyre'),
					'top_level'		=>	esc_html__("Top Level Page (parent of 0)",'blackfyre'),
					'parent'		=>	esc_html__("Parent Page (has children)",'blackfyre'),
					'child'			=>	esc_html__("Child Page (has parent)",'blackfyre'),
				);
			...
			case "page_template" :
				...
				$choices = array(
					'default'	=>	esc_html__("Default Template",'blackfyre'),
				);
			...
			case "post" :
				...
				unset( $post_types['page'], $post_types['attachment'], $post_types['revision'] , $post_types['nav_menu_item'], $post_types['blackfyre']  );
			...
			case "post_category" :
				...
				{
				  $choices[$cat_id->term_id] = $cat_id->name;
				}
			...
			case "post_status" :
				...
				$choices = array(
					'publish'	=> esc_html__( 'Publish', 'blackfyre' ),
					'pending'	=> esc_html__( 'Pending Review', 'blackfyre' ),
					'draft'		=> esc_html__( 'Draft', 'blackfyre' ),
					'future'	=> esc_html__( 'Future', 'blackfyre' ),
					'private'	=> esc_html__( 'Private', 'blackfyre' ),
					'inherit'	=> esc_html__( 'Revision', 'blackfyre' ),
					'trash'		=> esc_html__( 'Trash', 'blackfyre' )
				);
			...
			case "user_type" :
				...
				if( is_multisite() )
				{
					$choices['super_admin'] = esc_html__('Super Admin', 'blackfyre');
				}
			...
			case "ef_taxonomy" :
				...
				$choices = array('all' => esc_html__('All', 'blackfyre'));
			...
			case "ef_user" :
				...
				$choices = array_merge( array('all' => esc_html__('All', 'blackfyre')), $wp_roles->get_names() );
	...
	*/
	...
	{
		// validate
		if( !isset($_POST['post_type']) || $_POST['post_type'] != 'blackfyre' )
		{
			return $name;
		}
