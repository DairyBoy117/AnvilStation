...
function get_field_reference( $field_name, $post_id ) {
	...
	// cache
	$found = false;
	$cache = wp_cache_get( 'field_reference/post_id=' .  $post_id . '/name=' .  $field_name, 'blackfyre', false, $found );
	...
	// set cache
	wp_cache_set( 'field_reference/post_id=' .  $post_id . '/name=' .  $field_name, $return, 'blackfyre' );
...
function the_field( $field_name, $post_id = false ) {
	...
	echo wp_kses($value, $blackfyre_allowed);
}
...
add_shortcode( 'blackfyre', 'acf_shortcode' );
...
function acf_form( $options = array() )
{
	...
	$defaults = array(
		...
		'html_before_fields' => '',
		'html_after_fields' => '',
		'submit_value' => esc_html__("Update", 'blackfyre'),
		'updated_message' => esc_html__("Post updated", 'blackfyre'),
	);
	...
	// display form
	if( $options['form'] ): ?>
	<form <?php if($options['form_attributes']){foreach($options['form_attributes'] as $k => $v){echo esc_attr($k) . '="' . $v .'" '; }} ?>>
	<?php endif; ?>
    ...
    $blackfyre_allowed = wp_kses_allowed_html( 'post' );
	// html before fields
	echo wp_kses($options['html_before_fields'],$blackfyre_allowed);
    ...
    $blackfyre_allowed = wp_kses_allowed_html( 'post' );
	// html after fields
	echo wp_kses($options['html_after_fields'], $blackfyre_allowed);
	...
	<!-- Submit -->
	<div class="field">
		<input type="submit" value="<?php echo wp_kses($options['submit_value'], $blackfyre_allowed); ?>" />
	</div>
	<!-- / Submit -->
	<?php endif; ?>
