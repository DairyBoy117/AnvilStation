...
class WP_ClanWars_Widget extends WP_Widget {
    ...
    {
        $widget_ops = array('classname' => 'widget_clanwars', 'description' => esc_html__('ClanWars widget', 'blackfyre'));
        parent::__construct('clanwars', esc_html__('ClanWars', 'blackfyre'), $widget_ops);

        $this->default_settings = array('title' => esc_html__('ClanWars', 'blackfyre'),
                'show_limit' => 10,
                'hide_title' => false,
                'hide_older_than' => '1m',
                'visible_games' => array());
    ...
    function widget($args, $instance) {
        ...
        foreach($_games as $g) {
            $m = $wpClanWars->get_match(array('status' => array('active', 'done'),'from_date' => $from_date, 'game_id' => $g->id, 'limit' => $instance['show_limit'], 'order' => 'desc', 'orderby' => 'date', 'sum_tickets' => true));
            if(sizeof($m)) {
                $games[] = $g;
...
<ul class="clanwar-list">
	...
	<?php
		foreach($matches as $elementKey => $match){
			$timestamp = mysql2date('U', $match->date);
			$is_upcoming = $timestamp > $now;
			if($match->status == 'active' && $is_upcoming){
				unset($matches[$elementKey]);
				$matchesnew[] = $match;
		...
		if(!empty($matches)){
		$matchesnew = array_reverse($matchesnew);

		$matches = array_merge($matchesnew, $matches);
		}
    ...
    ?>
        ...
        <p>
            <input class="checkbox" name="<?php echo esc_attr($this->get_field_name('hide_title')); ?>" id="<?php echo esc_attr($this->get_field_id('hide_title')); ?>" value="1" type="checkbox" <?php checked($instance['hide_title'], true)?>/> <label for="<?php echo esc_attr($this->get_field_id('hide_title')); ?>"><?php esc_html_e('Hide title', 'blackfyre'); ?></label>
        </p>
