...
if($nmfilemanager -> group_id == 0){
	if ( isset( $_GET['shareable'] ) && isset( $_GET['shareable'] ) == 'yes' ){
		$args = array(
				'orderby'		=> 'post_title',
				'order'         => 'ASC',
...
if ( class_exists ( 'NM_WP_FileManager_Addon' ) && $nmfilemanager -> group_id == 0 ) {
	...
	echo '<a title="My Files" href="'. remove_query_arg( array('files', 'shareable')) .'"><i class="fa fa-files-o"></i></a> &nbsp;';
	printf( __('<a title="Shared Files" href="%s"><i class="fa fa-share-alt"></i></a></span><p style="clear: both; margin: 0;"></p></div>', 'nm-filemanager'), esc_url($urlSharedFiles) );
	...
	$post_files = new WP_Query($args);
	echo '</span><p style="clear: both; margin: 0;"></p></div>';
}
