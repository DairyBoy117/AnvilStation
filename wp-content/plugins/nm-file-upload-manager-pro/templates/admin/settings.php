...
<h2>
	<?php printf(__("%s", 'nm-filemanager'), $this->plugin_meta['name']);?>
</h2>
<div id="filemanager-tabs" class="wrap">
	...
	<form id="nm-admin-form-<?php echo $id?>" class="nm-admin-form-filemanager">
	...
	<div id="<?php echo $id?>">
		...
		<?php foreach($options['meat'] as $key => $data){
			...
			?>
				<tr valign="top">
					<th scope="row" width="25%"><?php printf(__('%s', 'nm-filemanager'), $label);?>
	...
	?>
	<p class="the-button"><button class="button button-primary"><?php _e('Save settings', 'nm-filemanager')?></button><span class="nm-saving-settings"></span></p>
	</form>
