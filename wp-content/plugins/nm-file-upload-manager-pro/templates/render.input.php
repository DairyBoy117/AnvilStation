...
foreach($existing_meta as $key => $meta)
		{
			...
			switch($type)
			{
				...
				case 'file':
					...
					$label_select = ($meta['button_label_select'] == '' ? __('Select files', 'nm-filemanager') : $meta['button_label_select']);
					/*$label_upload = ($meta['button_label_upload'] == '' ? __('Upload files', 'nm-filemanager') : $meta['button_label_upload']);*/
					...
					case 'image':
						...
						printf( __('<label for="%1$s">%2$s</label><br />', 'nm-filemanager'), $name, $field_label );
