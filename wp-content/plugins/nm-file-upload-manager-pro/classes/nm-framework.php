...
class NM_Framwork_V1_filemanager{
		...
		if($this -> plugin_scripts){
			foreach($this -> plugin_scripts as $script){
				...
				if( $script['type'] == 'js'){
					...
					$in_footer = (isset($script['in_footer']) ? $script['in_footer'] : NULL);
					
					wp_enqueue_script($this->plugin_meta['shortname'].'-'.$script['script_name'], $this->plugin_meta['url'].$script['script_source'], $depends, '3.0', $in_footer);
	...
	*/
	function get_option($key){
		...
		$the_option = (isset($this -> plugin_settings[ $full_key ]) ? $this -> plugin_settings[ $full_key ] : '');
